...
class CSVLogger(object):
    def __init__(self):
        ...
        self.headers = {'commits': ['commit_id',
                                    'title',
                                    'data',
                                    'committed_date',
                                    'committer_name',
...
class FileLogger(LoggingBase):
    def __init__(self, log_file=None):
        ...
        self.handler = logging.handlers.WatchedFileHandler("gitlab_watchman.log")
        self.logger.addHandler(self.handler)
        # self.logger.setLevel(logging.DEBUG)

    def log_notification(self, log_data, scope, detect_type, severity):
        self.handler.setFormatter(self.notify_format)
        self.logger.warning(json.dumps(log_data), extra={'scope': scope, 'type': detect_type, 'severity': severity})
